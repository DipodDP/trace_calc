...
class CoordinateParser:
    ...
    def parse(self, text: str) -> list[Coordinates]:
        ...
        for line in lines:
            ...
            tokens = re.findall(r"[+-]?\d+(?:\.\d+)?|[NSEWСВЮЗ]|[^\s;]+", line, re.IGNORECASE)

            i = 0
            while i < len(tokens):
                # Try to parse DMS: (d, m, s, [h])
                if i + 2 < len(tokens) and self._is_number(tokens[i]) and self._is_number(tokens[i+1]) and self._is_number(tokens[i+2]):
                    ...
                    if '.' not in d_str and '.' not in m_str:
                        ...
                        if not (0 <= m < 60 and 0 <= s < 60):
                            raise ValueError("Invalid DMS coordinate: minutes or seconds out of range")
        ...
        if len(all_coords) % 2 != 0:
            raise ValueError(f"Found an odd number of coordinate values: {len(all_coords)}")
